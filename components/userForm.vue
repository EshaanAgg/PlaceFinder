      ...
      <div class="field">
        <div class="two fields">
          <div class="field">
            ...
            <select v-model="type">
              <option
                v-for="location in typeValues.locations"
                :key="location"
                :value="location"
              >
                {{ capitalise(location) }}
...
const coordinatesDisplay = computed(
  () =>
    `${lat.value.toString().substring(0, 5)} Lat, ${lng.value
      .toString()
      .substring(0, 5)} Lng`
);
...
const findClosebyButtonPressed = async () => {
  ...
  categories.categories.forEach((c) => {
    if (c.includes(type.value) && filteredCategories.length <= 94)
      filteredCategories.push(c);
  });

  const URL = `https://api.geoapify.com/v2/places?categories=${filteredCategories.join(
    ","
  )}&filter=circle:${coordinatesAPISearch.value},${
    radius.value * 1000
  }&bias=proximity:${coordinatesAPISearch.value}&limit=30&apiKey=${
    config.GEOAPIFY_KEY
  }`;
  ...
  if (error.value) {
    ...
    places.value = fetchedPlaces.value.features.map((f) => ({
      ...f.properties,
